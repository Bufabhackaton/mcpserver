...
//
// Canonical dev path: bufab-mcp/scripts/validate.mjs
...
/**
 * Locate bufab-mcp `dist/index.js` for spawning `ui_export`. Resolution order:
 * 1. `BUFAB_MCP_DIST` — absolute path to `index.js` inside `dist/`
 * 2. `BUFAB_MCP_ROOT` — bufab-mcp package root (expects `dist/index.js` under it)
 * 3. Walk up from this file for `bufab-mcp/dist/index.js`
 * 4. `../dist/index.js` — when this file lives in `bufab-mcp/scripts/`
 */
function resolveMcpDistEntry() {
  const distFromEnv = process.env.BUFAB_MCP_DIST?.trim();
  if (distFromEnv && existsSync(distFromEnv)) return distFromEnv;

  const rootFromEnv = process.env.BUFAB_MCP_ROOT?.trim();
  if (rootFromEnv) {
    const p = resolvePath(rootFromEnv, "dist", "index.js");
    if (existsSync(p)) return p;
  }

  let dir = __dirname;
  for (let i = 0; i < 24; i++) {
    const cand = resolvePath(dir, "bufab-mcp", "dist", "index.js");
  ...
  const legacy = resolvePath(__dirname, "..", "dist", "index.js");
  if (existsSync(legacy)) return legacy;

  return null;
}

function loadGuidelinesFromMcp(mcpBinary) {
