...
```json
{
  "mcpServers": {
    "bufab-waf-mcp-proxy": {
      "command": "node",
      "args": ["${workspaceFolder}/bufab-waf-mcp-proxy/dist/index.js"],
      "env": {
...
Use `command`: `node`, `args`: `["/absolute/path/to/bufab-waf-mcp-proxy/dist/index.js"]`, and the same `env` keys as below. See **`mcp-config.example.json`** in this directory for a template with optional LanceDB path overrides.
